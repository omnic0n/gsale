-- Safe migration for categories table
-- This handles the auto-increment constraint properly
...
-- Check if id column is already auto-increment
SET @is_auto_increment = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'categories' 
    AND COLUMN_NAME = 'id' 
    AND EXTRA LIKE '%auto_increment%');

-- Only modify id column if it's not already auto-increment
SET @sql = (SELECT IF(
    @is_auto_increment = 0,
